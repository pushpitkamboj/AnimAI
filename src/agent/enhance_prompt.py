...
def enhanced_prompt(state: State):
    prompt = """   
        ...
        The use of LaTeX is strictly forbidden. You must never include MathTex, dollar signs, or any LaTeX syntax such as \theta, \pi, \frac or \sqrt{}. If any formula appears, write it using normal text or Unicode characters. For example, instead of writing $a^2 + b^2 = c^2$, write Text("a² + b² = c²"). If you detect any LaTeX content, automatically replace it with a simple text version. Any form of LaTeX usage will cause errors in the runtime, so ensure it is completely avoided.
